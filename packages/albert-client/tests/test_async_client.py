...
class TestAsyncClientInitialization:
    ...
    def test_init_without_api_key_raises_error(self, base_url):
        ...
        with pytest.raises(ValueError, match="Albert API key is required"):
            AsyncAlbertClient(base_url=base_url)

    def test_default_base_url(self, api_key):
        """Test default base URL is Albert production."""
        client = AsyncAlbertClient(api_key=api_key)
        assert "albert.api.etalab.gouv.fr/v1" in client.base_url


class TestAsyncOpenAICompatibleEndpoints:
    """Test async OpenAI-compatible endpoint passthrough."""
    ...
    @respx.mock
    async def test_chat_completion_call(self, api_key, base_url):
        """Test that async chat.completions.create works."""
        client = AsyncAlbertClient(api_key=api_key, base_url=base_url)
...
class TestAsyncContextManager:
    ...
    async def test_context_manager(self, api_key, base_url):
        ...
        # Mock endpoint (strip trailing slash from base_url)
        respx.post(f"{base_url.rstrip('/')}/chat/completions").mock(
            return_value=Response(
                200,
                json={
